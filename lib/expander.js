...
var Controllers = {
  ...
  ADS1015: {
    /*
    // Connect ADDR PIN to Ground (or nothing), VDD, SDA, or SCL to change i2c address (see datasheet table 5)
    | ADDR PIN | Address |
    | -------- | ------- |
    | Ground   | 0x48    |
    | VDD      | 0x49    |
    ...
    */
    ...
    REGISTER: {
      value: {
        AIN: [0x4000, 0x5000, 0x6000, 0x7000],
        // samples per second
        FREQUENCY_128: 0x0000, // 1 sample = 7,8 ms
        FREQUENCY_250: 0x0020, // 1 sample = 3,9 ms
        FREQUENCY_490: 0x0040, // 1 sample = 2,0 ms
        FREQUENCY_920: 0x0060, // 1 sample = 1,1 ms
        FREQUENCY_1600: 0x0080, // 1 sample = 0,6 ms
        FREQUENCY_2400: 0x00A0,
        FREQUENCY_3300: 0x00C0, // DEFAULT
        // gain (voltage ref)
        GAIN_6144: 0x0000, // 2/3 // DEFAULT for 5V (DEFAULT voltage)
        GAIN_4096: 0x0200, // 1 // DEFAULT for 3V3
        GAIN_2048: 0x0400, // 2
        GAIN_1024: 0x0600, // 4
        GAIN_512: 0x0800, // 8
        GAIN_256: 0x0A00, // 16
    ...
    initialize: {
      value: function(opts) {
        ...
        this.voltage = 1000 * (opts.voltage || 5);
        if (opts.gain) {
          this.gain = opts.gain;
        } else {
          // auto adjust gain based on voltage
          if (this.voltage >= 4096) {
            this.gain = 6144;
          } else if (this.voltage >= 2048) {
            this.gain = 4096;
          } else if (this.voltage >= 1024) {
            this.gain = 2048;
          } else if (this.voltage >= 512) {
            this.gain = 1024;
          } else if (this.voltage >= 256) {
            this.gain = 512;
          } else {
            this.gain = 256;
          }
        }
        this.frequency = opts.frequency || 3300;
        this.frequencies = [128, 250, 490, 920, 1600, 2400, 3300];
        this.bitShift = this.REGISTER.BIT_SHIFT;

        this.name = "ADS1" + (this.REGISTER.BIT_SHIFT ? "0" : "1") + "15";
        this.gains = (this.name === "ADS1015") ? [256, 512, 1024, 2048, 4096, 6144] : [8, 16, 32, 64, 128, 250, 475, 860];

        if (this.REGISTER["FREQUENCY_" + this.frequency] === undefined) {
          throw new Error("Expander `" + this.name + "` does not support `frequency` value: `" + this.frequency + "`. Supported values are: `" + this.frequencies.join("`, `") + "`");
        }
        if (this.REGISTER["GAIN_" + this.gain] === undefined) {
          throw new Error("Expander `" + this.name + "` does not support `gain` value: `" + this.gain + "`. Supported values are: `" + this.gains.join("`, `") + "`");
        }

        this.maxValue = Math.min(Math.round(this.RESOLUTION.ADC * this.voltage / this.gain) - 1, this.RESOLUTION.ADC);
    ...
    analogRead: {
      value: function(pin, callback) {
        ...
        var read = function() {
          ...
          var delay = ((1.0 / this.frequency) + 0.0001) * 1000;
          config |= this.REGISTER["FREQUENCY_" + this.frequency];
          config |= this.REGISTER.AIN[pin.analogChannel];
          config |= this.REGISTER["GAIN_" + this.gain];
          // set "single shot" mode
          config |= 0x8000;

          var self = this;

          this.io.i2cWrite(this.address, this.COMMANDS.ANALOG_WRITE, [(config >> 8) & 0xFF, config & 0xFF]);
          this.board.wait(delay, function() {
            self.io.i2cReadOnce(self.address, self.COMMANDS.ANALOG_READ, 2, function(bytes) {
              var raw = (((bytes[0] << 8) | bytes[1]) >> self.bitShift);
              raw = Fn.fmap(raw, 0, self.maxValue, 0, self.RESOLUTION.ADC);
              raw = Fn.constrain(raw, 0, self.RESOLUTION.ADC);
              handler(pinIndex, raw);
            }.bind(self));
          });
...
Controllers.ADS1115 = Object.assign({}, Controllers.ADS1015, {
  REGISTER: {
    value: {
      AIN: [0x4000, 0x5000, 0x6000, 0x7000],
      // samples per second
      FREQUENCY_8: 0x0000, // 1 sample = 125.0 ms
      FREQUENCY_16: 0x0020, // 1 sample = 62.5 ms
      FREQUENCY_32: 0x0040, // 1 sample = 31.2 ms
      FREQUENCY_64: 0x0060, // 1 sample = 15.6 ms
      FREQUENCY_128: 0x0080, // 1 sample = 7.8 ms
      FREQUENCY_250: 0x00A0, // 1 sample = 4.0 ms
      FREQUENCY_475: 0x00C0, // 1 sample = 2.1 ms
      FREQUENCY_860: 0x00E0, // DEFAULT // 1 sample = 1.2 ms
      // gain (voltage ref)
      GAIN_6144: 0x0000, // 2/3 // DEFAULT for 5V (DEFAULT voltage)
      GAIN_4096: 0x0200, // 1 // DEFAULT for 3V3
      GAIN_2048: 0x0400, // 2
